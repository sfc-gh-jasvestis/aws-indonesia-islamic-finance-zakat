...
export default function HomePage() {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 grid-cols-1">
        <Chart
          ...
          title="Zakat Collection (Monthly)"
        />
        <Chart
          ...
          title="Distribution by Asnaf Category"
        />
      </div>
        </div>
      </div>
  ...
  const domainTab1 = (
    <div className="space-y-6">
      ...
      <Chart
        ...
        type="area"
        xKey="x"
        yKeys={[{ key: 'y', name: 'Impact Index' }]}
        title="Beneficiary Impact Score"
        height={400}
      />
    </div>
  );
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Chart
          ...
          type="pie"
          xKey="label"
          yKeys={[{ key: 'value', name: 'Share %' }]}
          title="Digital vs Traditional Collection"
        />
        <ActionMemo
          persona={{ name: 'KH. Abdullah Gymnastiar', role: 'BAZNAS Deputy Chair' }}
          context={{}}
          onGenerate={async () => ({
            subject: 'Action Required',
            body: 'AI-generated recommendation based on current data patterns and predicted trends.',
            urgency: 'HIGH',
            actions: ['Launch Ramadan digital zakat campaign', 'Expand payroll deduction partnerships', 'Verify beneficiary eligibility for 12K pending applications'],
          })}
        />
      </div>
    </div>
  ...
  const askAiTab = (
    <div className="h-[600px]">
      <AskAI
        title="Ask AI"
        sampleQuestions={[
          'Which provinces are below zakat collection target?',
          'Show the impact of digital collection channels',
          'What is the optimal distribution mix by asnaf?',
        ]}
        mode="sql"
  ...
  const tabs = [
    ...
    { id: 'domain-1', label: 'Impact Analytics', icon: '📈', content: domainTab1 },
    { id: 'domain-2', label: 'Collection Optimization', icon: '⚡', content: domainTab2 },
